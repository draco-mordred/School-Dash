...
export enum DepartmentName {
  ...
  hematologyAndBloodTransfusion = "Hematology and Blood Transfusion",
  anatomicPathology = "Anatomic Pathology",
  microbiology = "Microbiology",
  chemicalPathology = "Chemical Pathology",
  clinicalParmacologyAndTherapeutics = "Clinical Pharmacology and Therapeutics",
...
export enum DepartmentCode {
  medicine = "MED",
  pediatrics = "PAE",
  obstetricsAndGynecology = "OBG",
  surgery = "SUR",
  psychiatry = "PSY",
  earNoseAndThroat = "ORL",
  anaesthesiology = "ANE",
  radiology = "RAD",
  ophthalmology = "OPH",
  dermatology = "DER",
  communityMedicine = "COM",
  hematologyAndBloodTransfusion = "HEM",
  microbiology = "MIC",
  chemicalPathology = "CHP",
  clinicalParmacologyAndTherapeutics = "PHA",
  anatomicPathology = "PAT",
  familyMedicine = "FAM",
  orthopaedics = "ORT",
  forensicMedicine = "FOR",
}

// Metadata for each department with name, code, and departmentID
export const DEPARTMENTS_METADATA: Record<
  DepartmentName,
...
export type DepartmentUnitEntry =
  | string
  | {
      id: string;
      name: string;
...
export type DepartmentUnitsData = {
  id: string;
  name: string;
  postingType?: string;
  rotationDurationWeeks: number;
  //current unit for when a student is undergoing a clinical rotation in this department in a specific unit
  currentUnit: string[];
  // Units under this department, categorized as active and reserve for rotation scheduling.
  units: {
    active: DepartmentUnitEntry[];
    reserve: DepartmentUnitEntry[];
    // History of units that have been used in the past for rotations, but are no longer active or reserve.
    history?: DepartmentUnitEntry[];
  };
};

export const DEPARTMENT_UNITS: Partial<Record<DepartmentName, DepartmentUnitsData>> = {
  [DepartmentName.obstetricsAndGynecology]: {
...
export const getDepartmentUnits = (name: DepartmentName) => DEPARTMENT_UNITS[name] ?? null;

export const getDepartmentUnitsByCode = (code: string) => {
  const departmentName = DEPARTMENT_NAMES.find(
    (name) => DEPARTMENTS_METADATA[name as DepartmentName].code === code
  );
  return departmentName ? DEPARTMENT_UNITS[departmentName as DepartmentName] ?? null : null;
};

export type DepartmentCourseDefinition = {
  title: string;
  units: number;
  courseID: string;
  code: string;
...
export const DEPARTMENT_COURSES: Partial<Record<DepartmentName, DepartmentCourseDefinition[]>> = {
  [DepartmentName.pediatrics]: [
    {
...
export const getDepartmentCourses = (name: DepartmentName) => DEPARTMENT_COURSES[name] ?? [];

export const getDepartmentCoursesByCode = (code: string) => {
  const departmentName = DEPARTMENT_NAMES.find(
    (name) => DEPARTMENTS_METADATA[name as DepartmentName].code === code
  );
  return departmentName ? DEPARTMENT_COURSES[departmentName as DepartmentName] ?? [] : [];
};
...
export const getDepartmentMetadata = (name: DepartmentName) => DEPARTMENTS_METADATA[name];

// Get all departments as array of objects (for seeding, forms, etc.)
export const getAllDepartments = () =>
  DEPARTMENT_NAMES.map((name) => ({
    ...DEPARTMENTS_METADATA[name as DepartmentName],
  }));