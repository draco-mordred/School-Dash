...
export const completeAcademicClockByClass = async (req, res) => {
    try {
        ...
        if (!clock)
            return res.status(404).json({ message: "Academic clock not found" });
        ...
        if (adminUsers.length > 0) {
            await Promise.all(adminUsers.map((user) => createNotificationIfUnique({
                userId: user._id,
                role: "admin",
