...
export const generateAttendanceForClassSession = async (req, res) => {
    try {
...
export const getStudentNotificationsSummary = async (req, res) => {
    try {
        const userId = req.user._id;
        ...
        const addIdsFromPeriods = (periods) => {
            for (const p of periods || []) {
                if (p?.subject)
                    subjectIds.add(String(p.subject));
                if (p?.lecturer)
                    lecturerIds.add(String(p.lecturer));
            }
        };
...
export const getCourseClassAttendance = async (req, res) => {
    try {
        const userId = req.user._id;
        const summary = await Attendance.aggregate([
            { $match: { lecturer: userId } },
...
export const approveExcusedAbsence = async (req, res) => {
    try {
...
export const getClassSessionAttendance = async (req, res) => {
    try {
...
export const bulkUpdateAttendance = async (req, res) => {
    try {
        ...
        const results = await Promise.all(updates.map(async ({ attendanceId, status, notes, lecturerApproval, hodApproval }) => {
            ...
            const record = await Attendance.findByIdAndUpdate(attendanceId, updateData, { returnDocument: 'after', runValidators: true });
            return record;
        }));
        await logActivity({
            userId,
...
// @route   POST /api/attendance/generate
// @access  Private (Admin/Teacher)
export const triggerAttendanceGeneration = async (req, res) => {
    try {
        const { courseId, classId, academicYearId, date } = req.body;
        if (!courseId || !classId || !academicYearId || !date) {
...
// @access  Private (Admin/Teacher)
export const checkTimetableExists = async (req, res) => {
    try {
...
// @access  Private (Admin/Teacher)
export const getAllAttendanceLists = async (req, res) => {
    try {
        const userId = req.user._id;
        const userRole = req.user.role;
        const { classId, courseId, date } = req.query;
...
// @access  Private (Admin/Teacher)
export const getSubjectsAttendance = async (req, res) => {
    try {
        const userId = req.user._id;
        const summary = await Attendance.aggregate([
            { $match: { lecturer: userId } },
...
// @access  Private (Admin/Teacher)
export const getWeeklyCourseAttendance = async (req, res) => {
    try {
        const userId = req.user._id;
        const userRole = req.user.role;
...
export const verifyClinicalClockIn = async (req, res) => {
    try {
        ...
        const geoEvaluation = await validateClinicalPresence({ latitude: studentLatitude, longitude: studentLongitude }, { latitude: unit.latitude, longitude: unit.longitude }, 150 // Strict 150-meter restriction geofence around ward perimeter
        );
