...
import { getAllDepartments, getDepartmentUnitsByCode, DEPARTMENT_UNITS, DEPARTMENT_COURSES, } from "../constants/departments";
/*
...
const findOrCreateDepartment = async (identifier) => {
    if (!identifier)
        return null;
    ...
    if (!departmentDoc) {
        ...
        if (constantsDept) {
            departmentDoc = await Department.findOneAndUpdate({ code: constantsDept.code }, {
...
const deriveUnitCode = (name) => String(name)
    ...
    .slice(0, 4)
    .toUpperCase() || "UNIT";
const getNormalizedDepartmentValue = (value) => {
    if (!value)
        return "";
...
const isUserInDepartment = (user, departmentDoc) => {
    if (!user || !departmentDoc)
        return false;
...
const getClassCourseDocuments = async (classId) => {
...
const validateDepartmentLecturers = async (lecturerIds, departmentDoc) => {
...
const findOrCreateUnit = async (departmentDoc, unitIdentifier) => {
    if (!unitIdentifier)
        return null;
...
const syncUnitsFromConstants = async () => {
    ...
    await Promise.all(departments.map(async (constDept) => {
        ...
        if (!departmentDoc)
            return;
        const normalizeUnitName = (unitEntry) => typeof unitEntry === "string"
        ...
        await Promise.all(unitNames.map(async (name, index) => {
            ...
            if (!cleanName)
                return;
            await Unit.findOneAndUpdate({ name: cleanName, department: departmentDoc._id }, {
                name: cleanName,
...
export const addCourseSubject = async (req, res) => {
    try {
        ...
        const existingSubject = (topLevelCourse.subjects ?? []).some((s) => String(s.subjectUID) === String(subjectUID) ||
...
//  @access  Private (Admin/Teacher/Unit)
export const deleteEmbeddedSubject = async (req, res) => {
    try {
        ...
        let subdoc = topLevelCourse.subjects.id
            ? topLevelCourse.subjects.id(subjectId)
            : null;
        if (!subdoc) {
            // Try match by embedded subject UID, subjectID, name or code
            subdoc = (topLevelCourse.subjects ?? []).find((s) => String(s._id) === String(subjectId) ||
    ...
    try {
        ...
        for (const c of topLevelCourses) {
            ...
            for (const s of subjects) {
                if (search) {
                    const matches = String(s?.name ?? "").toLowerCase().includes(search.toLowerCase()) ||
...
// -----------------------------
export const updateCourseSubjects = async (req, res) => {
    try {
        ...
        if (academicYearId)
            updateData.academicYear = academicYearId;
        ...
        const updated = await Course.findByIdAndUpdate(req.params.id, updateData, { returnDocument: "after", runValidators: true });
        const userId = req.user?._id;
        if (userId) {
            await logActivity({
                userId,
...
// -----------------------------
export const deleteCourseSubjects = async (req, res) => {
    try {
...
// -----------------------------
export const deduplicateClassCourses = async (req, res) => {
    try {
...
export const seedDepartments = async (req, res) => {
    try {
        ...
        const results = await Promise.all(departmentsData.map((dept) => Department.findOneAndUpdate({ code: dept.code }, { name: dept.name, code: dept.code, departmentID: dept.departmentID }, { upsert: true, returnDocument: "after" })));
        const userId = req.user?._id;
        if (userId) {
            await logActivity({
                userId,
...
const syncDepartmentsFromConstants = async () => {
    ...
    await Promise.all(constantDepartments.map(async (constDept) => {
        await Department.findOneAndUpdate({ code: constDept.code }, {
            name: constDept.name,
            code: constDept.code,
...
const normalizeDepartmentPayload = (raw) => {
...
export const getDepartmentConstants = async (req, res) => {
    try {
        return res.json({
