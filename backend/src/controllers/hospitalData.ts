 ...
 */
export const createHospitalUnit = async (req: Request, res: Response) => {
  try {
 ...
 */
export const listHospitalUnits = async (req: Request, res: Response) => {
  try {
    const limit = Math.min(parseInt(req.query.limit as string) || 20, 100);
    const skip = parseInt(req.query.skip as string) || 0;
    const department = req.query.department as string;
    const category = req.query.category as string;
    const umbrella = req.query.umbrella as string;
 ...
 */
export const getHospitalUnit = async (req: Request, res: Response) => {
  try {
 ...
 */
export const updateHospitalUnit = async (req: Request, res: Response) => {
  try {
    const { unitId } = req.params;
 ...
 */
export const listHospitalStaff = async (req: Request, res: Response) => {
  try {
    const limit = Math.min(parseInt(req.query.limit as string) || 20, 100);
    const skip = parseInt(req.query.skip as string) || 0;
    const department = req.query.department as string;
    const designation = req.query.designation as string;
    const systemRole = req.query.systemRole as string;
 ...
 */
export const updateHospitalStaff = async (req: Request, res: Response) => {
  try {
    const { staffId } = req.params;
...
export const bulkImportStaff = async (req: Request, res: Response) => {
  try {
    ...
    for (const data of staffData) {
      try {
        await HospitalStaffModel.updateOne(
