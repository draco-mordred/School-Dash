...
//   try {
//     const userId = (req as any).user._id;
//     //data from the expected request body based on the ILogbookEntry interface
//     const { rotationId, academicYearId, styudentId, type, details, attendance } = req.body;
...
//     //we need to check if it exists in the database before saving it to avoid duplicates
//     const existingEntry = await LogbookEntry.findOne({
//       student: userId,
//       rotationId,
//       academicYearId,
//       type,
//       "details.date": details.date, // Assuming details has a date field
//     });

//     if (existingEntry) {
//       return res.status(400).json({ message: "Logbook entry already exists for this date and type." });
//     }
