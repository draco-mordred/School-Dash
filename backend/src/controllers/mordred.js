...
const isInsightRole = (role) => permittedInsightRoles.has(String(role ?? "").trim().toLowerCase());
const handleAdminSystemAction = async (action, user) => {
...
export const saveChatMessage = async (req, res) => {
    try {
        ...
        if (!savedLog)
            return res.status(404).json({ message: "Message link not found." });
...
export const mordredsWords = async (req, res) => {
    try {
        ...
        if (!apiKey) {
            ...
            return res.status(200).json(buildMordredFallbackResponse("missing credentials", message, studentContext, userRole));
        }
        try {
        ...
        `,
            ...
            if (mordredDecision.shouldEscalate) {
                try {
                    const assignedStaff = await routeTaskToStaff(studentContext.department, "is_available_for_escalations", req.user._id);
                    await inngest.send({
                    ...
                    if (adminUsers.length > 0) {
                        await Promise.all(adminUsers.map((admin) => createNotificationIfUnique({
                            userId: admin._id,
                            role: "admin",
            ...
            return res.status(200).json(buildMordredFallbackResponse(error?.message || "AI request failed", message, studentContext, userRole));
        }
    }
    catch (error) {
...
export const trackMordredPerformance = async (req, res) => {
    try {
...
export const testWhatsAppAlert = async (req, res) => { };
export const dynamicAIInsights = async (req, res) => {
    try {
        const userRole = String(req.user?.role ?? "").trim().toLowerCase();
