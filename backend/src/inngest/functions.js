...
// Your new function:
export const generateTimeTable = inngest.createFunction({ id: "Generate-Timetable",
    triggers: {
    ...
    const classIdValue = typeof classId === "object"
        ...
        : classId;
    const academicYearIdValue = academicYearId ?? (typeof academicYear === "object"
    ...
    const contextData = await step.run("fetch-class-context", async () => {
        // Fetch class
        const classData = await ClassModel.findById(classIdValue).populate('courses');
        if (!classData)
...
//Next generate exam
export const generateExam = inngest.createFunction({ id: "Generate-Exam",
    triggers: {
        event: "exam/generate"
    }
}, async ({ event, step }) => {
...
export const generateAttendance = inngest.createFunction({ id: "Generate-Attendance",
    triggers: {
    ...
    const classData = await step.run("fetch-class-students", async () => {
        ...
        if (!cls)
            throw new NonRetriableError(`Class not found: ${classId}`);
        return cls;
    });
    ...
    const timetableData = await step.run("fetch-timetable-schedule", async () => {
        ...
        if (matchingPeriods.length === 0) {
            const availableSubjects = daySchedule.periods
                .map((p) => p.subject?.name ?? p.subject?.code ?? "Unknown")
                .filter(Boolean);
            const hint = availableSubjects.length > 0
                ? ` Available courses on ${dayName}: ${[...new Set(availableSubjects)].join(", ")}.`
                : "";
            throw new NonRetriableError(`NO_PERIOD: No period found for the selected course on ${dayName}. Please verify the course was added to the ${dayName} schedule in the timetable.${hint}`);
        }
    ...
    const createdRecords = await step.run("create-attendance-records", async () => {
        ...
        const records = await Promise.all(studentIds.map((studentId) => Attendance.create({
            student: studentId,
            lecturer,
            course: courseId,
            class: classId,
...
// ─── Bulk User Upload ─────────────────────────────────────────────────────────
export const bulkCreateUsers = inngest.createFunction({ id: "Bulk-Create-Users", triggers: { event: "users/bulk-create" } }, async ({ event, step }) => {
...
export const rotationNotify = inngest.createFunction({ id: "Rotation-Notify",
    triggers: {
 ...
 */
export const automaticPostingNotification = inngest.createFunction({ id: "Mordred-Auto-Posting-Assignment",
    triggers: {
 ...
 */
export const mordredTicketSentry = inngest.createFunction({ id: "Mordred-Ticket-Escalation-Sentry",
    triggers: {
...
export const whatsappLectureAlert = inngest.createFunction({ id: "mordred-whatsapp-lecture-alert",
    triggers: {
