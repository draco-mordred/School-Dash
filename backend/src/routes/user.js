...
import { login, registerUser, registerPublic, approvePendingUser, updateUser, deleteUser, logoutUser, getUserProfile, getUsers, getUserById, bulkUploadUsers, extractFromPDF, extractFromImage, isFirstUser, } from "../controllers/user";
...
userRoutes.get("/", protect, authorize(["admin", "teacher", "parent", "student", "unitconsultant"]), getUsers);
userRoutes.post("/:id/approve", protect, authorize(["admin"]), approvePendingUser);
userRoutes.get("/:id", protect, authorize(["admin", "teacher", "parent", "unitconsultant"]), getUserById);
...
userRoutes.delete("/delete/:id", protect, authorize(["admin", "teacher", "unitconsultant"]), deleteUser);
