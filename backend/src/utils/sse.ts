...
const heartbeats: WeakMap<Response, NodeJS.Timer> = new WeakMap();

export function addSSEClient(req: any, res: Response) {
  // Set headers for SSE
  ...
  if (!userId) {
    ...
    const set = clients.get('_anon') || new Set<Response>();
    set.add(res);
...
function reqOnClose(res: Response, cb: () => void) {
  try {
    // prefer socket close
    (res as any).on && (res as any).on('close', cb);
    (res as any).on && (res as any).on('finish', cb);
    // fallback: when connection ends
  } catch {}
}

export function sendSSE(event: string, data: any, userId?: string) {
