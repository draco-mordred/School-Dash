...
export default function TeacherCourses() {
  ...
  const [courseToAddSubject, setCourseToAddSubject] = useState<Course | null>(
    null,
  );
  const { user } = useAuth();
  ...
  const [sortBy, setSortBy] = useState<
    "name-asc" | "name-desc" | "subjects-high" | "subjects-low"
  >("name-asc");
  ...
  const [createDepartments, setCreateDepartments] = useState<
    Array<{ _id: string; name: string; departmentID: string; code: string }>
  >([]);
  ...
  >([]);
  const [createAcademicYears, setCreateAcademicYears] = useState<
  ...
  // Hover and expand animations disabled for this view
  ...
  const [subjectsPanelCourseId, setSubjectsPanelCourseId] = useState<
    string | null
  >(null);
  const [subjectsPanelVisible, setSubjectsPanelVisible] = useState(false);
  const [subjectsPanelPosition, setSubjectsPanelPosition] = useState<{
    left: number;
    top: number;
    height: number;
  } | null>(null);
  ...
  const subjectsPanelAnchorRef = useRef<{
    left: number;
    top: number;
    width: number;
    height: number;
  } | null>(null);
  const cardTransforms = useRef<
    Record<
      string,
      {
        dx: number;
  ...
  useEffect(() => {
    const loadCourseMeta = async () => {
      try {
        ...
        setCreateAcademicYears(
          Array.isArray(data.academicYears) ? data.academicYears : [],
        );
      } catch (error) {
  ...
  const canCreateCourse = ["admin", "teacher", "unitconsultant"].includes(
    user?.role ?? "",
  );
    ...
    try {
      ...
      if (selectedClassId) {
        const { data } = await api.get(
          `/courses?class=${selectedClassId}&topLevel=true`,
        );
  ...
  useEffect(() => {
    ...
    const updatePanelPosition = () => {
      ...
      const contentDesired = Math.min(
        panelHeight,
        (subjects.length || 0) * itemHeight + headerHeight,
      );

      if (anchor) {
        const left = Math.min(
          anchor.left + anchor.width + 16,
          viewportWidth - panelWidth - 16,
        );
        const top = Math.max(
          16,
          Math.min(anchor.top, viewportHeight - panelHeight - 16),
        );
        setSubjectsPanelPosition({
          left,
          top,
          height: Math.min(
            Math.max(anchor.height + 24, contentDesired),
            panelHeight,
          ),
        });
        return;
      ...
      setSubjectsPanelPosition({
        left,
        top: rect.top,
        height: Math.min(Math.max(rect.height, contentDesired), panelHeight),
      });
    };
  ...
  useEffect(() => {
    const fetchUnitResidentClasses = async () => {
      try {
        ...
        const allClasses = Array.isArray(response.data.classes)
    ...
    const fetchCoursesForClass = async () => {
      ...
      try {
        ...
        if (courseResponse.status === "fulfilled") {
          setCourses(
            Array.isArray(courseResponse.value.data.courses)
              ? courseResponse.value.data.courses
              : [],
          );
        } else {
        ...
        if (timetableResponse.status === "fulfilled") {
          setClassSchedule(
            Array.isArray(timetableResponse.value.data.schedule)
              ? timetableResponse.value.data.schedule
              : [],
          );
        } else {
          console.warn(
            "Unable to load class timetable:",
            timetableResponse.reason,
          );
          setClassSchedule([]);
        }
      } catch (err: unknown) {
  ...
  const startExpand = (courseId: string) => {
    ...
    const baseTargetWidth =
      vw < 768 ? Math.min(vw * 0.92, 560) : Math.min(vw * 0.42, 640);
    ...
    const targetLeft = Math.min(
      Math.max(16, pointerX - rect.width * 0.3),
      maxLeft,
    );
    const targetTop = Math.min(
      Math.max(16, pointerY - rect.height * 0.3),
      Math.max(16, vh - expandedHeight - 16),
    );
    const dx = targetLeft - rect.left;
    const dy = targetTop - rect.top;
    const originX = Math.min(
      Math.max(pointerX - rect.left, 24),
      rect.width - 24,
    );
    const originY = Math.min(
      Math.max(pointerY - rect.top, 24),
      rect.height - 24,
    );
    ...
    cardTransforms.current[courseId] = {
      dx,
      dy,
      scale,
      anchorX: targetWidth / 2,
      anchorY: expandedHeight / 2,
    };
  ...
  const [courseSubjectOptions, setCourseSubjectOptions] = useState<
    SubjectOption[]
  >([]);
  ...
  useEffect(() => {
      ...
      try {
        ...
        const options = allCourses.flatMap((courseDoc) => {
          const sameDepartment =
            String(courseDoc.department?._id ?? "") ===
            String(courseToEdit.department?._id ?? "");
          if (!sameDepartment) return [];

          return (
            Array.isArray(courseDoc.subjects) ? courseDoc.subjects : []
          ).map((subject) => ({
            _id: String(
              subject._id ??
                `${courseDoc._id}-${subject.subjectID ?? subject.name}`,
            ),
            name: subject.name,
            code: subject.code ?? "",
        ...
        const uniqueOptions = Array.from(
          new Map(options.map((option) => [option._id, option])).values(),
        ) as SubjectOption[];
    ...
    const departmentIdentifier =
      courseToEdit.department?.departmentID ??
      courseToEdit.department?.code ??
      courseToEdit.department?._id ??
      "";
    const loadDepartmentTeachers = async () => {
      ...
      try {
        const { data } = await api.get(
          `/users?role=teacher&page=1&limit=200&department=${encodeURIComponent(departmentIdentifier)}`,
        );
    ...
    const existingHodId = Array.isArray(courseToEdit.lecturer)
      ? typeof courseToEdit.lecturer[0] === "string"
        ? courseToEdit.lecturer[0]
        : (courseToEdit.lecturer[0]?._id ?? "")
      : "";
    ...
    const frameId = window.requestAnimationFrame(() => {
      setCourseEditForm({
        ...
        academicYear:
          typeof courseToEdit.academicYear === "object"
            ? (courseToEdit.academicYear?._id ?? "")
            : "",
        ...
        selectedSubjectIds: (courseToEdit.subjects ?? []).map(
          (subject) => subject._id,
        ),
      });
    });
  ...
  const submitCourseEdit = async () => {
    ...
    try {
      const selectedSubjects = courseSubjectOptions
        .filter((subject) =>
          courseEditForm.selectedSubjectIds.includes(subject._id),
        )
        .map((subject) => ({
          name: subject.name,
          ...
          isActive: true,
          semester:
            courseToEdit.subjects?.find(
              (current) => current._id === subject._id,
            )?.semester ?? null,
        }));
  ...
  const submitAddSubject = async () => {
    ...
    try {
      ...
      const looksLikeObjectId =
        ...
        /^[0-9a-fA-F]{24}$/.test(departmentIdentifier);
      ...
      await api.post(`/courses/${courseToAddSubject._id}/subjects`, {
        subject: {
          ...
          lecturer: addSubjectForm.lecturerId
            ? [addSubjectForm.lecturerId]
            : [],
          students: [],
          isActive: true,
        },
      ...
      >
        <DialogContent className="sm:max-w-[520px]">
          <DialogHeader>
            ...
            <DialogDescription>
              Update course metadata for this course.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="grid gap-3 py-2">
            ...
            <Select
              ...
              onValueChange={(value) =>
                setCourseEditForm((p) => ({ ...p, academicYear: value }))
              }
            >
              <SelectTrigger>
            ...
            <Select
              ...
              onValueChange={(value) =>
                setCourseEditForm((p) => ({ ...p, hodId: value }))
              }
            >
              <SelectTrigger>
              ...
              <div className="max-h-48 space-y-2 overflow-y-auto pr-1">
                {courseSubjectOptions.length === 0 ? (
                  <p className="text-sm text-muted-foreground">
                    No department-scoped subjects are available for this course
                    yet.
                  </p>
                ) : (
                      ...
                      >
                        ...
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground">
                            {subject.code || subject.subjectID || "No code"}
                          </p>
                        </div>
                      </label>
                    );
      ...
      >
        <DialogContent className="sm:max-w-[560px]">
          <DialogHeader>
            ...
            <DialogDescription>
              Create a top-level course and assign it to the currently selected
              class.
            </DialogDescription>
          </DialogHeader>
          ...
          <form onSubmit={handleCreateCourseSubmit} className="space-y-4 py-2">
            ...
            <Select
              value={createCourseForm.department}
              onValueChange={(value) =>
                setCreateCourseForm((prev) => ({ ...prev, department: value }))
              }
            >
              <SelectTrigger>
            ...
            <Select
              value={createCourseForm.unit}
              onValueChange={(value) =>
                setCreateCourseForm((prev) => ({ ...prev, unit: value }))
              }
            >
              <SelectTrigger>
            ...
            <Select
              value={createCourseForm.academicYearId}
              onValueChange={(value) =>
                setCreateCourseForm((prev) => ({
                  ...prev,
            ...
            {selectedClass && (
              <div className="rounded-lg border border-primary/20 bg-primary/5 p-3 text-sm">
                ...
                <p className="text-xs text-muted-foreground">
                  Academic year: {selectedClass.academicYear?.name ?? "Unknown"}
                </p>
                <Link
                  to={`/classes?selected=${selectedClass._id}`}
      ...
      >
        <DialogContent className="sm:max-w-[520px]">
          <DialogHeader>
            ...
            <DialogDescription>
              Add an embedded subject to the selected course.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="grid gap-3 py-2">
            <Input
              ...
              onChange={(e) =>
                setAddSubjectForm((p) => ({ ...p, name: e.target.value }))
              }
            />
            <Input
              ...
              onChange={(e) =>
                setAddSubjectForm((p) => ({ ...p, code: e.target.value }))
              }
            />
            <Input
              placeholder="Course Name"
            ...
            <Select
              ...
              onValueChange={(value) =>
                setAddSubjectForm((p) => ({ ...p, lecturerId: value }))
              }
            >
              <SelectTrigger>
            ...
            <Select
              ...
              onValueChange={(value) =>
                setAddSubjectForm((p) => ({ ...p, semester: value }))
              }
            >
              <SelectTrigger>
        ...
        <div className="rounded-3xl border border-border bg-card p-6 shadow-sm">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="flex h-14 w-14 items-center justify-center rounded-3xl bg-primary/10 text-primary">
                {institution?.logoUrl ? (
                  ...
                  <span className="text-xl font-semibold">
                    {String(institution?.shortName ?? institution?.name ?? "?")
                      .slice(0, 2)
                      .toUpperCase()}
                  </span>
                )}
            ...
            <div className="grid gap-1 text-right">
              ...
              <p className="text-lg font-semibold">
                {selectedClass?.academicYear?.name ?? "Not set"}
              </p>
            </div>
          </div>
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {loadingCourses ? (
            ...
            <>
              ...
              {subjectsPanelCourseId &&
                expandedCourseId === subjectsPanelCourseId &&
                subjectsPanelPosition && (
                  <div
                    ...
                    style={{
                      left: `${subjectsPanelPosition.left}px`,
                      top: `${subjectsPanelPosition.top}px`,
                      maxHeight: `min(70vh, ${subjectsPanelPosition.height}px)`,
                    }}
                  >
                    <div className="mb-3 flex items-center justify-between">
                      <div>
                        <p className="text-sm font-semibold">
                          {selectedCourse?.name ?? "Course subjects"}
                        </p>
                        <p className="text-xs text-muted-foreground">
                          {selectedCourse?.code}
                        </p>
                ...
                {filteredAndSortedCourses.map((course) => {
                  ...
                  return (
                    <Card
                      ...
                      onMouseMove={() => updateCardPointer(course._id, 0, 0)}
                    ...
                    >
                      ...
                      <CardContent className="space-y-3">
                        ...
                        <div className="text-xs text-muted-foreground">
                          ...
                          <p>
                            {course.department?.head?.name ?? "Not assigned"}
                          </p>
                        </div>
                        {course.semester && (
                        ...
                        <div className="space-y-1 pt-2 text-sm text-muted-foreground">
                          {loadingSchedule ? (
                            ...
                            (() => {
                              const scheduleItems = getCourseSchedule(
                                course._id,
                              );
                              return scheduleItems.length > 0 ? (
                                <div className="space-y-1">
                                  {scheduleItems
                                    ...
                                    .map((period, index) => (
                                      <div
                                        key={`${period.day}-${period.startTime}-${index}`}
                                ...
                                <p>
                                  No timetable sessions found for this course.
                                </p>
                              );
                            })()
                          )}
                          ...
                          <Button
                            ...
                            onClick={(e) => {
                              ...
                              const ok = window.confirm(
                                "Remove this course from the selected class?",
                              );
                              if (!ok) return;
                              void (async () => {
                                try {
                                  await api.delete(
                                    `/classes/${selectedClassId}/courses/${course._id}`,
                                  );
                                  // refresh
                                  if (selectedClassId) {
                                    const [courseResponse, timetableResponse] =
